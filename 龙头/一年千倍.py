# 克隆自聚宽文章：https://www.joinquant.com/post/50575
# 标题：再次进行优化，年化90%，确保长测有效
# 作者：悠然贱男
...
def buy(context):
    ...
    for s in g.target_list:
        ...
        avg_price_increase_value = prev_day_data['money'][0] / prev_day_data['volume'][0] / prev_day_data['close'][
            0] * 1.1 - 1
        if avg_price_increase_value < 0.07 or prev_day_data['money'][0] < 5e8 or prev_day_data['money'][0] > 20e8:
            continue
    ...
    if len(qualified_stocks) != 0 and context.portfolio.available_cash / context.portfolio.total_value > 0.3:
        print('———————————————————————————————————')
        send_message('今日选股：' + ','.join('%s%s' % (s, get_security_info(s).display_name) for s in qualified_stocks))
        print('一进二：' + ','.join('%s%s' % (s, get_security_info(s).display_name) for s in gk_stocks))
        print('弱转强：' + ','.join('%s%s' % (s, get_security_info(s).display_name) for s in rzq_stocks))
        print('今日选股：' + ','.join('%s%s' % (s, get_security_info(s).display_name) for s in qualified_stocks))
        print('———————————————————————————————————')

        # 计算每天的买入资金限制
        daily_buy_limit = context.portfolio.total_value * 0.5
        # 计算每只股票的买入金额
        value = daily_buy_limit / len(qualified_stocks)
        for s in qualified_stocks:
            # 下单
            if context.portfolio.available_cash / current_data[s].last_price > 100:
                order_value(s, min(value, context.portfolio.available_cash), MarketOrderStyle(current_data[s].day_open))
                print('买入股票：%s%s' % (s, get_security_info(s).display_name))
                print('———————————————————————————————————')
...
def filter_kcbj_stock(initial_list):
    return [stock for stock in initial_list if not (
                stock.startswith('70') or stock.startswith('69') or stock.startswith('4') or stock.startswith(
            '8') or stock[:2] == '88')]
