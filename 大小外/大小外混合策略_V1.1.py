# 克隆自聚宽文章：https://www.joinquant.com/post/51716
# 标题：24年收益414.16%最大回撤9.80%优化大小外择时
# 作者：Qtrader2

# 克隆自聚宽文章：https://www.joinquant.com/post/51707
# 标题：市场宽度优化大小外择时，年化提高6%，平均回撤降低7%
# 作者：成都小宝总
...
# 20万资金
# 2024年 策略收益414.16 基准收益 Alpha Beta Shareoe 最大回撤9.80
# 100万资金
# 2024年 策略收益414.16 基准收益 Alpha Beta Shareoe 最大回撤9.80
...
def initialize(context):
    ...
    # 5 1 257
    run_weekly(clear, 5, '9:30')
    run_daily(monthly_adjustment, '9:30')
    # run_daily(stop_loss, '14:00')
    run_daily(stop_loss, '14:30')
    run_daily(stop_loss, '14:50')
...
def monthly_adjustment(context):
    today = context.current_dt
    dt_last = context.previous_date
    # width_data=get_industry_width(dt_last,5,'sw_l1')
    # if width_data['全市场'][1]-width_data['全市场'][0]>30:
    #     g.singal=='big'
    target_list = []
    print(g.singal)
    if g.singal == 'big':
