...
# 高频使用循环模式，低频使用定时
text = {
    "账户": "8888888888",  # 填写你自己的资金账户
    "账户类型": "STOCK",  # 账户类型STOCK/CREDIT 普通账户/融资融券
    "买入价格编码": 5,  # 5最新价、6买1价、7买2价(不支持组合)、8买3价(不支持组合)、9买4价(不支持组合)、10买5价(不支持组合)
    "卖出价格编码": 5,  # 5最新价、4卖1价、3卖2价、2卖3价、1卖4价、0卖5价
    "黑名单": ['600031.SH', '600111.SH', '513100.SH'],  # 黑名单里面的标的，不会买入也不会卖出
    "聚宽跟单": "跟单原理",
    "服务器设置": "服务器跟单设置",
    ...
    "跟单设置": "跟单设置***********",
    "账户跟单比例": 1,  # 这里设置跟单比例：假如聚宽10W，账户跟单比例设置1，如果只跟5W，设置0.5
    "多策略用逗号隔开": "多策略用逗号隔开********",
    "组合名称": ["小市值"],  # 改成策略名称，中文
    "组合授权码": ["apitest"],  # 改成自己的授权码
    "组合跟单比例": [0.5],  # 这里的跟单比例，用来配置多个聚宽跟单！
    "不同策略间隔更新时间": 0,
    "时间设置": "时间设置********",
    "交易时间段": 4,
    "交易开始时间": 9,
...
def init(c):
    ...
    c.url = text['服务器']
    c.port = text['端口']
    c.is_open_id_log = text['是否开启id记录']
    print('300M独享超高速跟单************服务器{} 端口{}'.format(c.url, c.port))
    # 定时模式
    ...
    # 交易检查函数0.5分钟一次
    c.run_time("run_check_trader_func", "30nSecond", "2024-07-25 13:20:00")
    # 撤单了重新下单5分钟一次
...
def get_del_buy_sell_data(c, name='测试1', password='apitest'):
    '''
    处理交易数据获取原始数据
    '''
    test = text['是否开启测试']
...
def get_trader_data(c, name='测试', password='apitest', zh_ratio=0.1):
    '''
    获取交易数据
    组合的跟单比例
    '''
            ...
            if df.shape[0] > 0:
                for stock, amount, trader_type in zip(df['证券代码'].tolist(), df['下单数量'].tolist(),
                    ...
                    try:
                        ...
                        if test == '是':
                            amount = test_amount * adjust_ratio * zh_ratio
                            value = amount * price
                        else:
...
def start_trader_on(c, name='测试1', password='xg_st', zh_ratio=0.1):
    '''
    开始下单
    '''
            ...
            for stock, av_amount, hold_amount, amount, maker, in zip(sell_df['证券代码'].tolist(),
                ...
                if stock not in c.del_trader_list:
                    if stock not in a.log_id:
                        ...
                        if av_amount >= 10:
                            try:
                                ...
                                if check_is_sell(c, c.account, c.account_type, stock=stock, amount=amount):
                                    passorder(c.sell_code, 1101, c.account, str(stock), c.sell_price_code, 0,
                        ...
                        print('{} {} 在id记录不卖出等待订单确认检查'.format(name, stock))
                else:
            ...
            for stock, amount, maker, in zip(buy_df['证券代码'].tolist(), buy_df['数量'].tolist(),
                ...
                if stock not in c.del_trader_list:
                    ...
                    if maker not in a.log_id:
                        ...
                        print('{} {} 在id记录不买入等待订单确认检查'.format(name, stock))
                else:
        ...
        for name, password in zip(name_list, password_list):
            ...
            if trader_log.shape[0] > 0:
                for stock, amount, trader_type, maker, oder_id in zip(trader_log['证券代码'].tolist(),
                    ...
                    price = get_price(c, stock)
                    # 未成交卖出
                    print('证券代码：{} 未成交数量{}交易类型{} 投资备注{} 订单id{}'.format(stock, amount, trader_type,
                                                                                          maker, oder_id))
                    if trader_type == 49:
                        cancel(oder_id, c.account, c.account_type, c)
                        passorder(c.sell_code, 1101, c.account, str(stock), c.sell_price_code, 0, int(amount),
                                  str(maker), 1, str(maker), c)
                        print('组合{} 撤单重新卖出标的{} 数量{} 价格{}'.format(name, stock, amount, price))
                    elif trader_type == 48:
                        cancel(oder_id, c.account, c.account_type, c)
                        passorder(c.buy_code, 1101, c.account, str(stock), c.buy_price_code, 0, int(amount), str(maker),
                                  1, str(maker), c)
                        print('组合{} 撤单重新买入标的{} 数量{} 价格{}'.format(name, stock, amount, price))
                    else:
                        print('组合{} 撤单重新交易未知的交易类型'.format(name))
            else:
                print('撤单了在下单组合{}没有委托数据'.format(name))
    else:
        print('撤单了重新下单没有委托数据')
...
class xg_jq_data:
    def __init__(self, url='http://server.588gs.cn',
                 port=2000,
