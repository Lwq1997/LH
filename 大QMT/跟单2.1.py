# encoding:gbk
'''
聚宽跟单QMT代码，需要聚宽信号代码联系微信xjip20
QMT代码参数为实盘参数，除了'组合授权码'需要填写之外，其他参数非必要勿改。
'''
import pandas as pd
import numpy as np
import talib
...
text = {
    ...
    "测试数量": 200,
    "跟单设置": "跟单设置***********",
    "账户跟单比例": 1,  # 聚宽如果是10W，这里跟单1就是10W，如果0.5就是5万。
    "多策略用逗号隔开": "多策略用逗号隔开********",
    "组合名称": ["小市值"],  # 策略名称
    "组合授权码": ["123456"],  # 授权码,微信xjip20获取
    "组合跟单比例": [1],  # 这里是设置多策略组合使用，默认无需改。
    "不同策略间隔更新时间": 0,
    "下单默认说明": "默认/金额/数量",
    "下单模式": "默认",  # 默认就好无需修改。
    "下单值": 1000,
...
def init(c):
    ...
    c.url = text['服务器']
    c.port = text['端口']
    print('无极提供数据支持************服务器{} 端口{}'.format(c.url, c.port))
    # 定时模式
