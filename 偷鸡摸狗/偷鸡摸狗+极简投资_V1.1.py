...
def initialize(context):
    ...
    g.research_mode = True  # 是否开启研究模式（不适用于回测）：关闭自动调仓，每个策略初始占比必须大于0
    g.strategys_values = pd.DataFrame(columns=["s1", "s2", "s3", "s4"])
    g.portfolio_value_proportion = [0, 0.5, 0.1, 0.2, 0.2]
...
def calculate_optimal_weights(context):
    ...
    # 定义边界：每个权重都大于等于0，没有上限
    bounds = [(0.05, None) for _ in initial_weights]

    # 优化问题
    result = minimize(
        negative_sharpe_ratio, initial_weights, method="SLSQP", constraints=constraints, bounds=bounds
    )

    # 输出最佳权重
    best_weights = result.x
    print('原始权重配比如下:', best_weights)

    # 调整权重
    adjusted_weights = round_weights(best_weights, decimals=2)

    print('优化后权重配比如下:', adjusted_weights)
    # print('调整前的权重:', g.portfolio_value_proportion)
    # g.portfolio_value_proportion = [0] + list(adjusted_weights)
    # print('调整后的权重:', g.portfolio_value_proportion)


def round_weights(weights, decimals=2):
    rounded = np.round(weights, decimals)
    diff = 1 - np.sum(rounded)
    # 找到最大的权重索引
...
class Strategy:
    ...
    def balance_subportfolios(self, context):
        ...
        # 仓位比例过高调出资金
        log.info(f'第{self.subportfolio_index}个仓位总金额:{value},可用金额:{cash},目标金额:{target},综上转移{min(value - target, cash)}到0号账户')
        ...
        # 仓位比例过低调入资金
        log.info(f'第{self.subportfolio_index}个仓位总金额:{value},目标金额:{target},目前0号账户可用金额{cash},综上从0号账户转移{min(value - target, cash)}到{self.subportfolio_index}号账户')
